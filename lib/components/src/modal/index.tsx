...
export const Modal = ({ onClose, onOpen, open, position, backdrop = true, children, className, ...props }: modalProps) => {
  ...
  return <handleCtx.Provider value={{ onClose: handleClose, onOpen: toogleOpen(handleOpen(!isOpen)) }}>
    {button}
    <dialog {...props} ref={ref} className={modalProps({ open: isOpen, position, className })} onClose={toogleOpen(onClose)}>
      {rest}
...
export const ModalBox = ({ className, ...props }: React.ComponentPropsWithRef<'div'>) => {
  return <div {...props} className={boxProps({className})} />
}
